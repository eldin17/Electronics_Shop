...
{
    public class OutOfStockProductState : BaseProductState
    {
        public OutOfStockProductState(DataContext context, IMapper mapper, IServiceProvider serviceProvider) : base(context, mapper, serviceProvider)
        {
        }
        ...
        public override async Task<DtoProduct> Restock(int id)
        {
            var dbObj = await _context.Products.Include(x => x.ProductColors).FirstOrDefaultAsync(x => x.Id == id);

            var stock = dbObj.ProductColors.Any(x => x.Stock > 0);
