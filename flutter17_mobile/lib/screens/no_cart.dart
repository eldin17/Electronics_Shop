...
class NoCartScreen extends StatelessWidget {
  ...
  NoCartScreen({super.key, this.productObj});

  @override
  Widget build(BuildContext context) {
    final _shoppingCartProvider = context.read<ShoppingCartProvider>();
    return Scaffold(
        ...
        child: Padding(
          ...
          child: Column(
            children: [
              ...
              const Spacer(flex: 2),
              ErrorInfo(
                title: "No Cart? No Worries!",
                description:
                    "Your cart doesn’t exist yet.\n Let’s spawn one and start your epic tech haul!",
                btnText: "Create My Shopping Cart",
                press: () async {
                  var shoppingCartObj = await _shoppingCartProvider
                      .add({'customerId': LoginResponse.currentCustomer!.id});

                  LoginResponse.currentCustomer?.shoppingCart = shoppingCartObj;
