...
import 'package:flutter17_mobile/widgets/payment_method_popup.dart';
import 'package:flutter_svg/flutter_svg.dart';
import 'package:provider/provider.dart';

class NoPaymentMethodsScreen extends StatefulWidget {
  const NoPaymentMethodsScreen({super.key});

  @override
  State<NoPaymentMethodsScreen> createState() => _NoPaymentMethodsScreenState();
}

class _NoPaymentMethodsScreenState extends State<NoPaymentMethodsScreen> {
          ...
          child: Column(
            children: [
              ...
              ErrorInfo(
                ...
                description:
                    "You haven't added any payment methods yet. Please add a method to proceed",
                btnText: "Add payment method",
                press: () {
                  showPaymentMethodPopup(
                    context,
                    initialCashSelected: false,
                    initialStripeSelected: false,
                    onConfirm: (cash, stripe) async {
                      print('Cash: $cash, Stripe: $stripe');
                      if (cash) {
                        ...
                        print(LoginResponse
                            .currentCustomer!.paymentMethods?.length);
                      }
                      if (stripe) {
                        var obj = await _paymentMethodProvider.add({
                          "type": "Card",
